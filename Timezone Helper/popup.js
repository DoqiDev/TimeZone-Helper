...
function loadTimezones() {
  chrome.storage.local.get(['timezones', 'theme'], (data) => {
...
function renderTimezones() {
  ...
  if (timezones.length === 0) {
    timezoneListEl.innerHTML = '<p style="text-align:center;opacity:0.6;">No timezone added</p>';
    return;
  }
  ...
  timezones.forEach(tz => {
    ...
    item.appendChild(tzLabel);
    item.appendChild(right);
    timezoneListEl.appendChild(item);
  });
}

function showTimezoneSelector() {
  if (document.getElementById('tz-select-wrapper')) return;

  const wrapper = document.createElement('div');
  wrapper.id = 'tz-select-wrapper';
  ...
  confirmBtn.style.background = '#4b9cff';
...
addBtn.addEventListener('click', showTimezoneSelector);
